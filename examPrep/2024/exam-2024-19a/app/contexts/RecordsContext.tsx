...
type RecordContextType = {
  ...
  updateRecord: (record: Item, id: number) => Promise<void>;
  ...
  addOwnerToDb: (owner: string) => Promise<void>;
  getOwnerDb: () => Promise<string>;
};
...
export default function RecordContextProvider({ children }: any) {
  ...
  const addOwnerToDb = async(owner: string) => {
    await saveOwnerToDb(dbConnection!, owner);
  }

  const getOwnerDb = async() => {
    const storedOwner = await getOwnerFromDb(dbConnection!);
    if(!storedOwner)
      return '';
  ...
  const updateRecord = async (record: Item, id: number) => {
    try {
      console.log("id: ", (id));
      record.id = id;
      await updateVehicle(record);
  ...
  const setupWebSocket = () => {
    ...
    socket.onmessage = async (event) => {
      ...
      const formattedMessage = `New Vehicle with owner ${receivedRecord.owner} model ${receivedRecord.model}, with status ${receivedRecord.status} has been added`;
  ...
  useEffect(() => {
    ...
    const connection = SQLite.openDatabaseSync("vehicles.db");
  ...
  return (
    <RecordContext.Provider
      value={{
        ...
        addOwnerToDb: addOwnerToDb,
        getOwnerDb: getOwnerDb,
      }}
    >
      {children}
    </RecordContext.Provider>
