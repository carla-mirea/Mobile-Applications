...
router.put('/updatevehicle', ctx => {
  const { id, model, status, capacity, owner, manufacturer, cargo } = ctx.request.body;

  const vehicle = vehicles.find(v => v.id == id);
  if (vehicle && model && status && capacity && owner && manufacturer && cargo) {
    vehicle.model = model;
    ...
    console.log(`Vehicle not found or invalid fields, id: ${id} model: ${model} status: ${status} capacity: ${capacity} owner: ${owner} manufacturer: ${manufacturer} cargo: ${cargo}`);
    ctx.response.body = { error: `Vehicle not found or invalid fields, id: ${id} model: ${model} status: ${status} capacity: ${capacity} owner: ${owner} manufacturer: ${manufacturer} cargo: ${cargo}` };
