...
export async function addRecordDb(dbConnection: SQLiteDatabase, record: Item) {
  ...
  return dbConnection?.runAsync(
    "INSERT OR REPLACE INTO Item (id, name, supplier, details, status, quantity, type) VALUES (?, ?, ?, ?, ?, ?, ?)",
    [record.id, record.name, record.supplier, record.details, record.status, record.quantity, record.type]
  );
}
...
export async function updateRecordDb(dbConnection: SQLiteDatabase, recordToUpdate: Item) {
  ...
  return dbConnection?.runAsync(
    `UPDATE Item 
     SET name = ?, supplier = ?, details = ?, status = ?, quantity = ?, type = ?
     WHERE id = ?`,
    [
      recordToUpdate.name,
