...
export default function RecordContextProvider({ children }: any) {
  ...
  const setupWebSocket = () => {
    ...
    socket.onmessage = async (event) => {
      ...
      console.log(receivedRecord.name);
      ...
      const formattedMessage = `New Order with supplier ${receivedRecord.supplier} named ${receivedRecord.name}$, with status ${receivedRecord.status} has been added`;
  ...
  useEffect(() => {
    ...
    const connection = SQLite.openDatabaseSync("supplies.db");
