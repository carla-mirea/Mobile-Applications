...
export async function addRecordDb(dbConnection: SQLiteDatabase, record: Item) {
  ...
  return dbConnection?.runAsync(
    "INSERT OR REPLACE INTO Item (id, title, author, genre, quantity, reserved) VALUES (?, ?, ?, ?, ?, ?)",
    [record.id, record.title, record.author, record.genre, record.quantity, record.reserved]
  );
}
...
export async function updateRecordDb(dbConnection: SQLiteDatabase, recordToUpdate: Item) {
  ...
  return dbConnection?.runAsync(
    `UPDATE Item 
     SET title = ?, author = ?, genre = ?, quantity = ?, reserved = ?
     WHERE id = ?`,
    [
      recordToUpdate.title,
      recordToUpdate.author,
      recordToUpdate.genre,
      recordToUpdate.quantity,
      recordToUpdate.reserved,
      recordToUpdate.id,
    ]
  );
}
    ...
    try {
      ...
      for (const operation of pendingOperations) {
        ...
        try {
          ...
          } else if (syncAction === "ADD" && payload) {
            const record = JSON.parse(payload) as Item;
            await postRecordToServer(record);
          }
